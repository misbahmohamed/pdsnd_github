...
def display_data(df):
    ...
    counter2 = 5
    while True:
        answer2 = input('if you want to see indivedual data choose (yes) to show 5 users or (no) to skip : ').lower()
        if answer2 == 'yes':
            i = 0
            while (i < 5):
...
def main():
    while True:
        ...
        while counter <= 2:
            answer=input("would you like to show informtion about {} ?('yes' or 'skip') : ".format(info_name[counter])).lower()
            if answer == "yes" :
